...
describe Proxmox do
  ...
  describe 'lxc_get' do
    it 'should get lxc vm list' do
      # First VM list
      stub_request(:get, 'http://localhost:8006/api2/json/nodes/localhost/lxc').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
          body: '{"data":[{"maxswap":536870912,"disk":404037632,"ip":"192.168.1.5","status":"running","netout":272,"maxdisk":4294967296,"maxmem":536870912,"uptime":3847,"swap":0,"vmid":"101","nproc":"10","diskread":0,"cpu":0.00183354942808597,"netin":0,"name":"test2.dummy.tld","failcnt":0,"diskwrite":0,"mem":21303296,"type":"lxc","cpus":1},{"maxswap":536870912,"disk":387186688,"ip":"192.168.1.1","status":"running","netout":272,"maxdisk":4294967296,"maxmem":536870912,"uptime":17120,"swap":0,"vmid":"100","nproc":"17","diskread":0,"cpu":0.000504170031344927,"netin":0,"name":"test.dummy.tld","failcnt":0,"diskwrite":0,"mem":27987968,"type":"lxc","cpus":1}]}'
        )

      # Second VM list
      stub_request(:get, 'http://localhost:8006/api2/json/nodes/otherone/lxc').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
          body: '{"data":[{"maxswap":536870912,"disk":404041728,"ip":"192.168.1.5","status":"running","netout":272,"maxdisk":4294967296,"maxmem":536870912,"uptime":6176,"swap":0,"vmid":"101","nproc":"10","diskread":0,"cpu":0.00161487378340585,"netin":0,"name":"test2.dummy.tld","failcnt":0,"diskwrite":0,"mem":21299200,"type":"lxc","cpus":1},{"maxswap":2147483648,"disk":0,"ip":"10.0.0.1","status":"stopped","netout":0,"maxdisk":10737418240,"maxmem":1073741824,"uptime":0,"swap":0,"vmid":"102","nproc":0,"diskread":0,"cpu":0,"netin":0,"name":"test3.other.domain","failcnt":0,"diskwrite":0,"mem":0,"type":"lxc","cpus":2},{"maxswap":536870912,"disk":387194880,"ip":"192.168.1.1","status":"running","netout":272,"maxdisk":4294967296,"maxmem":536870912,"uptime":19449,"swap":0,"vmid":"100","nproc":"17","diskread":0,"cpu":0.000589570582552814,"netin":0,"name":"test.dummy.tld","failcnt":0,"diskwrite":0,"mem":28282880,"type":"lxc","cpus":1}]}'
        )

      expect(@server1.lxc_get).to be_an_instance_of Hash
      expect(@server1.lxc_get.keys.sort).to be_eql %w(100 101)

      server2 = Proxmox::Proxmox.new('http://localhost:8006/api2/json/', 'otherone', 'root', 'secret', 'pam')
      expect(server2.lxc_get).to be_an_instance_of Hash
      expect(server2.lxc_get.keys.sort).to be_eql %w(100 101 102)
    end
  end

  describe 'lxc_post' do
    it 'should create a container' do
      # Create VM
      stub_request(:post, 'http://localhost:8006/api2/json/nodes/localhost/lxc').with(
      ...
      # Create the vm
      expect(@server1.lxc_post('ubuntu-10.04-standard_10.04-4_i386', 200)).to be_eql 'UPID:localhost:00051DA0:119EAABB:521CCB19:vzcreate:200:root@pam:'
      expect(@server1.lxc_post('ubuntu-10.04-standard_10.04-4_i386', 203, 'hostname' => 'vm1.domain.com', 'password' => 'secret', 'memory' => 512, 'swap' => 512, 'disk' => 4)).to be_eql 'UPID:localhost:00051DA0:119EAABC:521CCB19:vzcreate:203:root@pam:'
      expect(@server1.lxc_post('ubuntu-10.04-standard_10.04-4_i386', 204)).to be_eql 'NOK: error code = 500'
    end
  end

  describe 'lxc_delete' do
    it 'should delete lxc container' do
      # Delete VM
      stub_request(:delete, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
      ...
      stub_request(:delete, 'http://localhost:8006/api2/json/nodes/localhost/lxc/201').with(
        headers: @common_headers_in).to_return(
          status: 500,
          headers: @common_headers_out,
          body: 'NOK: error code = 500'
      ...
      # Delete the vm
      expect(@server1.lxc_delete(200)).to be_eql 'UPID:localhost:0005C1EB:11BAA4EB:521D12B8:vzdestroy:200:root@pam:'
      expect(@server1.lxc_delete(201)).to be_eql 'NOK: error code = 500'
    end
  end

  describe 'lxc_status' do
    it 'should get container status' do
      # VM Status
      stub_request(:get, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200/status/current').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
          body: '{"data":{"maxswap":268435456,"disk":0,"ip":"-","status":"stopped","ha":0,"netout":0,"maxdisk":9.44473296573929e+21,"maxmem":268435456,"uptime":0,"swap":0,"nproc":0,"diskread":0,"cpu":0,"netin":0,"name":"CT200","failcnt":0,"diskwrite":0,"mem":0,"type":"lxc","cpus":1}}'
        )

      expect(@server1.lxc_status(200)).to be_an_instance_of Hash
      expect(@server1.lxc_status(200)['status']).to be_eql 'stopped'
      expect(@server1.lxc_status(200)['cpus']).to be_eql 1
    end
  end

  describe 'lxc_start' do
    it 'should start vm' do
      stub_request(:post, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200/status/start').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
      ...
      expect(@server1.lxc_start(200)).to be_eql 'UPID:ks311324:0005D91C:11BE5277:521D1C23:vzstart:200:root@pam:'
    end
  end

  describe 'lxc_stop' do
    it 'should stop vm' do
      stub_request(:post, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200/status/stop').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
      ...
      expect(@server1.lxc_stop(200)).to be_eql 'UPID:ks311324:0005D91C:11BE5277:521D1C23:vzstop:200:root@pam:'
    end
  end

  describe 'lxc_shutdown' do
    it 'should shutdown vm' do
      stub_request(:post, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200/status/shutdown').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
      ...
      expect(@server1.lxc_shutdown(200)).to be_eql 'UPID:ks311324:0005D91C:11BE5277:521D1C23:vzshutdown:200:root@pam:'
    end
  end

  describe 'lxc_config' do
    it 'should get container config' do
      # VM config
      stub_request(:get, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200/config').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
      ...
      expect(@server1.lxc_config(200)).to be_an_instance_of Hash
      expect(@server1.lxc_config(200)['searchdomain']).to be_eql 'domain.com'
      expect(@server1.lxc_config(200)['ostemplate']).to be_eql 'ubuntu-10.04-standard_10.04-4_i386.tar.gz'
    end
  end

  describe 'lxc_config_set' do
    it 'should modify container config' do
      # VM config
      stub_request(:put, 'http://localhost:8006/api2/json/nodes/localhost/lxc/200/config').with(
        headers: @common_headers_in).to_return(
          status: 200,
          headers: @common_headers_out,
          body: '{"data":null}'
      ...
      @server1.lxc_config_set(200, 'searchdomain' => 'other.com')
    end
  end
end
